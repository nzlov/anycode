...
func TestMindMapCodexContextSeparatesRealtimeAndAsyncTools(t *testing.T) {
	...
	if err != nil || len(tools) != 2 || tools[0] != processdomain.DynamicToolMindMapSearch || tools[1] != processdomain.DynamicToolMindMapUpdate || !strings.Contains(guidance, "必须及时") || !strings.Contains(guidance, "不要尝试读取全图") || !strings.Contains(guidance, "一个节点只表达一个稳定概念") || !strings.Contains(guidance, "禁止将文件列表更新到节点内容里") || !strings.Contains(guidance, "project-root") || !strings.Contains(guidance, "禁止创建仅用于记录错误") {
