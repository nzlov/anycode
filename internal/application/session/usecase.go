...
const artifactPromptGuidance = "本卡片用于预览、下载或过程验证的图片、截图、PDF、音视频、压缩包和其他临时文件统一写入环境变量 `ANYCODE_ARTIFACT_DIR` 指向的目录。需要生图时直接使用 Codex 可用的图片生成能力，临时生成文件保存到该目录。任务需要纳入项目的正式素材，允许在用户请求或授权范围内复制、移动或生成到项目工作树，并遵循项目资源目录约定；已有授权无需重复确认。"
const realtimeMindMapPromptGuidance = "本项目已开启实时思维图。只在需要了解已有概念、关系或准备变更时调用 `mind_map_search`；query 使用 SQL 风格布尔表达式，查询词通过 AND/OR 组合，AND 优先于 OR，可用括号改变优先级，相邻查询词之间必须有操作符，例如 `workflow AND (runner OR retry)`。limit 通常取 3–5，不要读取全图、批量组合多次搜索，或把搜索与高输出命令放进同一次工具调用。搜索结果中的命中节点含完整信息和 Tag，关联节点仅为摘要，并返回可直接传给 `mind_map_update` 的 tagRevision。当需求、功能、决策或关联关系变化时，必须及时更新本卡片隔离图，无需请求用户批准；无图变化时不要调用更新工具。更新前对将变更的概念做一次窄搜索，优先复用已有节点，并把最近一次搜索的 tagRevision 原样传给 `mind_map_update`；仅在没有新鲜搜索结果或更新提示 revision 过期时调用 `mind_map_tags` 刷新。`mind_map_tags` 返回 JSON 文本，在同一次代码工具调用中读取字段前必须先解析。Agent 不得创建、更新、删除 Tag 节点或直接维护 Tag 关系；每个节点 upsert 必须携带完整的 Tag 名称列表，后端负责规范化 Tag、生成受控 ID、对账关系并清理孤儿 Tag，且只有 Tag 可以作为 `project-root` 的一级节点。节点标题不能为空，一个节点只表达一个稳定概念。引用实现代码的节点必须携带非空 files，逐项记录准确的文件、方法和起止行号；非代码节点不得携带文件位置，文件列表不得写入节点内容。唯一固定节点是标题为项目名、ID 为 `project-root` 的中心根节点，不得修改、删除或移动。禁止创建仅记录错误、失败、临时调试、交付状态、提交或测试结果的节点；结束时只清理本次任务创建或修改后已失效的此类节点及其关系，不得为全库清理执行宽泛搜索。思维图只保留稳定的项目结构、需求、功能、决策和关联关系。"
const asyncMindMapQueryPromptGuidance = "本项目使用异步思维图维护。当前常规会话只可调用 `mind_map_search` 查询确有助于当前任务的概念或关系。query 使用 SQL 风格布尔表达式，查询词通过 AND/OR 组合，AND 优先于 OR，可用括号改变优先级，相邻查询词之间必须有操作符，例如 `workflow AND (runner OR retry)`。limit 通常取 3–5，不要读取全图、批量组合多次搜索，或把搜索与高输出命令放进同一次工具调用。当前会话不得修改思维图；会话关闭后由独立分析任务整理并更新。"
...
func (s *Service) prepareMindMapClose(ctx context.Context, session domain.Session, reason domain.CloseReason, now time.Time) (*mindMapCloseAction, error) {
	...
	action := &mindMapCloseAction{project: project, session: session, updatedAt: now}
...
func (a *mindMapCloseAction) apply(ctx context.Context, repo mindmapdomain.Repository) error {
	...
	mindmapdomain.Touch(&graph, a.updatedAt)
	if err := repo.SaveGraph(ctx, graph, overlay.Changes); err != nil {
		return err
	}
	return repo.DeleteOverlay(ctx, sessionID)
}
