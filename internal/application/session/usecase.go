...
func (s *Service) closeSession(ctx context.Context, input CloseSessionInput) (DTO, error) {
	...
	if err := s.cancelPendingQuestions(ctx, session.ID, "session closed"); err != nil {
		return DTO{}, s.releaseClosePreparation(ctx, session, err)
	}
	...
	if strings.TrimSpace(session.BaseBranch) != "" && strings.TrimSpace(session.WorktreePath) != "" {
		if s.worktrees == nil {
			closeErr := apperror.New(apperror.CodeCloseFailed, apperror.CategoryInfraError, "session worktree manager is required").WithDetails(map[string]any{
				"sessionId": string(session.ID),
			})
			return DTO{}, s.releaseClosePreparation(ctx, session, closeErr)
		}
		exists, err := s.worktrees.Exists(ctx, session.WorktreePath)
		if err != nil {
			closeErr := apperror.Wrap(err, apperror.CodeCloseFailed, apperror.CategoryInfraError, "check session worktree existence failed").WithDetails(map[string]any{
				"sessionId": string(session.ID),
			}).WithRetryable(true)
			return DTO{}, s.releaseClosePreparation(ctx, session, closeErr)
		}
		...
		if exists {
			project, err = s.projects.Find(ctx, projectdomain.ID(session.ProjectID))
			if err != nil {
				return DTO{}, s.releaseClosePreparation(ctx, session, fmt.Errorf("find project for session worktree cleanup: %w", err))
			}
			if project.IsGit {
				if err := s.worktrees.Remove(ctx, session.WorktreePath); err != nil {
					closeErr := apperror.Wrap(err, apperror.CodeCloseFailed, apperror.CategoryInfraError, "remove session worktree failed").WithDetails(map[string]any{
						"sessionId": string(session.ID),
					}).WithRetryable(true)
					return DTO{}, s.releaseClosePreparation(ctx, session, closeErr)
				}
