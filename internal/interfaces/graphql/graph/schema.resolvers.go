...
// SaveMCPService is the resolver for the saveMCPService field.
func (r *mutationResolver) SaveMCPService(ctx context.Context, scope model.MCPScopeInput, name string, definition map[string]any, enabled *bool) (bool, error) {
	if r.UseCases.MCP == nil {
		return false, missingUseCase("mcp")
	}
	var d *mcpdomain.Definition
	if definition != nil {
		...
		if err := decoder.Decode(&d); err != nil {
			return false, fmt.Errorf("invalid MCP definition: %w", err)
		}
	}
	id := ""
	if scope.ID != nil {
	...
	err := r.UseCases.MCP.Save(ctx, mcpdomain.Scope{Kind: scope.Kind, ID: id}, name, d, enabled)
	return err == nil, err
}

// DeleteMCPService is the resolver for the deleteMCPService field.
func (r *mutationResolver) DeleteMCPService(ctx context.Context, scope model.MCPScopeInput, name string) (bool, error) {
	if r.UseCases.MCP == nil {
		return false, missingUseCase("mcp")
	}
	id := ""
	...
	err := r.UseCases.MCP.Delete(ctx, mcpdomain.Scope{Kind: scope.Kind, ID: id}, name)
	return err == nil, err
}

// CheckMCPService is the resolver for the checkMCPService field.
func (r *mutationResolver) CheckMCPService(ctx context.Context, scope model.MCPScopeInput, name string) (int, error) {
	if r.UseCases.MCP == nil {
		return 0, missingUseCase("mcp")
	}
	id := ""
	if scope.ID != nil {
		id = *scope.ID
	}
	return r.UseCases.MCP.Check(ctx, mcpdomain.Scope{Kind: scope.Kind, ID: id}, name)
}
...
func (r *mutationResolver) UpdateProjectMindMap(ctx context.Context, input model.UpdateMindMapInput) (*model.MindMapUpdateEvent, error) {
	...
	for _, operation := range input.Operations {
		var files *[]mindmapdomain.NodeFile
		if operation.Files != nil {
			// GLUE: gqlgen input pointers preserve omitted versus empty files, while the domain uses value objects.
			values := make([]mindmapdomain.NodeFile, 0, len(operation.Files))
			for _, item := range operation.Files {
				values = append(values, mindmapdomain.NodeFile{
		...
		operations = append(operations, mindmapapp.OperationInput{
			...
			Files: files, SourceID: sourceID, TargetID: targetID, Label: operation.Label,
		})
	}
	dto, err := r.UseCases.MindMaps.Apply(ctx, mindmapapp.UpdateInput{
	...
	return mapMindMapUpdate(dto), nil
}
...
func (r *mutationResolver) OpenSessionTerminal(ctx context.Context, sessionID string) (*model.Session, error) {
	...
	dto, err := r.UseCases.Sessions.OpenTerminal(ctx, sessiondomain.ID(sessionID))
	if err != nil {
		return nil, err
	}
	return mapSession(dto), nil
...
// McpServices is the resolver for the mcpServices field.
func (r *queryResolver) McpServices(ctx context.Context, scope model.MCPScopeInput) ([]*model.MCPService, error) {
	if r.UseCases.MCP == nil {
		return nil, missingUseCase("mcp")
	}
	id := ""
	if scope.ID != nil {
		id = *scope.ID
	}
	services, err := r.UseCases.MCP.List(ctx, mcpdomain.Scope{Kind: scope.Kind, ID: id})
	if err != nil {
		return nil, err
	}
	out := make([]*model.MCPService, 0, len(services))
	for _, v := range services {
		item := &model.MCPService{Name: v.Name, Enabled: v.Enabled, Source: v.Source, Overridden: v.Overridden}
		if v.Definition != nil {
			data, err := json.Marshal(v.Definition)
			if err != nil {
				return nil, err
			}
			if err := json.Unmarshal(data, &item.Definition); err != nil {
				return nil, err
			}
		}
		out = append(out, item)
...
func (r *queryResolver) ProjectMindMap(ctx context.Context, input model.MindMapPageInput) (*model.MindMapGraphPage, error) {
	if r.UseCases.MindMaps == nil {
		return nil, missingUseCase("mind maps")
	}
	dto, err := r.UseCases.MindMaps.GetPage(ctx, mindmapapp.GetPageInput{
		ProjectID: mindmapdomain.ProjectID(input.ProjectID), SessionID: mindmapdomain.SessionID(stringValue(input.SessionID, "")),
		NodeAfter: mindmapdomain.NodeID(stringValue(input.NodeAfter, "")), EdgeAfter: mindmapdomain.EdgeID(stringValue(input.EdgeAfter, "")),
		IncludeNodes: input.IncludeNodes, IncludeEdges: input.IncludeEdges, PageSize: input.PageSize,
	})
	if err != nil {
		return nil, err
	}
