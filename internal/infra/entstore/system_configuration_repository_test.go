...
func TestSystemConfigurationRepositoryPersistsAppearanceSettings(t *testing.T) {
	...
	if configuration.AgentMaxConcurrent != 2 || configuration.SendShortcut != setting.SendShortcutShiftEnter || configuration.MindMap.Layout != setting.MindMapLayoutRadial || configuration.BackgroundType != setting.BackgroundTypeBing || configuration.SolidTheme != setting.SolidThemeVermilion || configuration.WallpaperColorScheme != setting.WallpaperColorSchemeContent {
...
func TestSystemConfigurationMigrationAddsWritableRootsToExistingRow(t *testing.T) {
	...
	configuration, err := store.Settings().GetSystemConfiguration(ctx)
	if err != nil || configuration.AgentMaxConcurrent != 3 || len(configuration.AgentWritableRoots) != 0 || configuration.SendShortcut != setting.SendShortcutShiftEnter || configuration.MindMap.Layout != setting.MindMapLayoutRadial {
