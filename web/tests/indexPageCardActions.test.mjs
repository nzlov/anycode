...
test('overview keeps closed cards out of latest and folds them into history', () => {
  ...
  assert.equal(source.includes('createOverviewCardGroups(latestRows.value, historyRows.value)'), true);
});
...
test('history more link opens the sessions table with closed scope', () => {
  ...
  const sessionsSource = readFileSync(new URL('../src/pages/SessionsPage.vue', import.meta.url), 'utf8');
