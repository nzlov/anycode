...
AnyCode 是面向 Codex agent 的 Web 工作台。它把项目、会话卡片、隔离 worktree、可编排工作流和人工介入集中到一个浏览器界面中，适合同时跟踪多个编码任务。
...
- 归档产物并预览受支持的图片、PDF、音视频和文本；其他文件可下载，也可重新作为输入使用。
- 卡片关闭时会清理可变输出和归档产物，保留已删除审计记录；已复制为输入的附件继续保留。
...
1. 打开全局设置，在“项目”中选择 `/workspaces` 下的项目目录。
